...
<div class="col-6" id="{{$id ?? ''}}">
    <div class="card card-info">
        <div class="card-header p-2">
            <h3 class="card-title">@lang('Analytic::general.top_browsers')</h3>
        ...
        <div class="card-body table-responsive p-0" style="height: 395px;">
            <table class="table table-bordered table-head-fixed table-sm">
                <thead>
                    <tr>
                    <th style="padding: 10px;">@lang('Analytic::general.browser')</th>
                    <th style="width: 120px; padding: 10px;">@lang('Analytic::general.session')</th>
                    </tr>
                </thead>
                <tbody></tbody>
            </table>
...
<script>
    ...
    function getTopBrowser({!! $date_id ?? '' !!}_page) {
        data = {
        ...
        loadAjaxPost('{{ route('admin.ajax.get_top_browsers') }}', data, {
            beforeSend: function(){
            ...
            success:function(result){
                ...
                if (result.status == 1) {
                    ...
                    $.each(result.data, function(index, item) {
                        str += `
                            <tr>
                                <td style="padding: 5px 10px;">${item.browser}</td>
                                <td style="padding: 5px 10px;">${item.sessions}</td>
                            </tr>
                        `;
                    });
